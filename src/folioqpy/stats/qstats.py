...
from ..periods import DAILY, MONTHLY, QUARTERLY, WEEKLY, YEARLY, AnnualizationFactor
from .utils import (
    _adjust_returns,
    _aligned_series,
...
def gpd_var_calculator(
    ...
    if exceedance_n > 0 and shape_param > 0:
        ...
        result = threshold + (param_ratio * (math.pow(prob_ratio, -shape_param) - 1))
    return result
...
def summarize_paths(
    ...
    for num_std in cone_std:
        cone_bounds[float(num_std)] = cum_mean + cum_std * num_std
        cone_bounds[float(-num_std)] = cum_mean - cum_std * num_std

    cone_bounds = pd.DataFrame(cone_bounds)
